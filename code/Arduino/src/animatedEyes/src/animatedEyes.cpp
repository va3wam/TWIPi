...
#include <normalEye.h> // Using this to figure put process for using custom images for OLED
/***********************************************************************************************************
  Synopsis:
  This sketch is being used to come up with the most effective eye animations for a pair of OLED displays 
  that will be used on the TWIPi class robots. Maybe try the U8glib2 library rather than the Adafruit_GFX
  library since we are having so much trouble getting images to display properly. The approach we are going 
  to try is outlined at this site: 
  https://sandhansblog.wordpress.com/2017/04/16/interfacing-displaying-a-custom-graphic-on-an-0-96-i2c-oled/  
  which has some details on using GIMP to create images and export using the X Bitmap format (.xbm).
...
/*
  ...
  ------- ---------- ---------------------------------------------------------------------------------------
  1.0.0   2018-12-02 Code base created.
 ***********************************************************************************************************/
...
void drawNormalEyes(void) 
{
  ...
  rightOLED.drawBitmap(
    ...
    normal_eye, LOGO_WIDTH, LOGO_HEIGHT, 1);
  rightOLED.display();
